...
(defrule initializare
        (start)
        =>
        (printout t "Alegeti numarul de linii:"crlf)
        (bind ?n (read))
        (printout t "Alegeti numarul de coloane:" crlf)
        (bind ?m (read))
        (assert (size (linie ?n) (coloana ?m)))

        (printout t "Alegeti numarul de celule vii:" crlf)
        (bind ?o (read))

        (printout t "Alegeti numarul de iteratii:" crlf)
        (bind ?it (read))
        (assert (iteratii ?it))
        (assert (counter ?it))
        ...
        (loop-for-count (?cnt1 1 ?o) do
                (printout t "Elementul " ?cnt1 " linia: ")
                (bind ?x (read))
                (printout t "Elementul " ?cnt1 " coloana: ")
                (bind ?y (read)) 
                (assert (modifica ?x ?y))
        )
)
